\documentclass{article}

% https://manualdelatex.com/simbolos

% Language setting
% Replace `english' with e.g. `spanish' to change the document language
\usepackage[spanish]{babel}

% Set page size and margins
% Replace `letterpaper' with `a4paper' for UK/EU standard size
\usepackage[letterpaper,top=2cm,bottom=2cm,left=3cm,right=3cm,marginparwidth=1.75cm]{geometry}
\usepackage{titlesec}

% Useful packages
\usepackage{amsmath}
\usepackage{graphicx}
\usepackage[colorlinks=true, allcolors=blue]{hyperref}
\usepackage{tikz}
\usepackage{parskip}
\usepackage{listings} 
% Define OQL as a custom language (if needed)
\lstdefinelanguage{OQL}{
    keywords={CREATE TYPE, FROM, WHERE, GROUP, BY, HAVING, ORDER, INSERT, UPDATE, DELETE},
    sensitive=false, % Case insensitive
    morecomment=[l]{--}, % Line comment
    morecomment=[s]{/*}{*/}, % Block comment
    morestring=[b]", % Strings inside double quotes
}
\usetikzlibrary{shapes.geometric, arrows}

% we want ER + above/below + left/right
\usetikzlibrary{er,positioning}
\tikzstyle{startstop} = [rectangle, minimum width=3cm, minimum height=1cm,text
centered, draw=black, fill=blue!15]
\tikzstyle{arrow} = [thick]
\setlength{\parindent}{0pt}

\title{Resumen de Diseño de Bases de Datos}
\author{You}
\begin{document}
\maketitle

\include{Unidades/Unidad01}
\include{Unidades/Unidad02}
\include{Unidades/Unidad03}
\include{Unidades/Unidad04}
\include{Unidades/Unidad05}
\include{Unidades/Unidad06}
\include{Unidades/Unidad07}

\end{document}
